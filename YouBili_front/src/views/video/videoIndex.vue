<template>
  <div>
    <el-drawer :visible.sync="drawer" :with-header="false" direction="ltr" size="250px" title="我是标题">
      <Sidebar></Sidebar>
    </el-drawer>
    <Handder></Handder>
    ...
    <div class="idxs">
      <div style="margin-top: 10px">
        <el-row :gutter="10">
          <el-col :span="16">
            <div>
              <h1 class="title">{{ allVideoCount.title }}</h1>
            </div>
            <div>
              <span><i class="el-icon-video-play"></i>{{ allVideoCount.favoriteCount }}</span>
              <span style="margin-left: 20px"><i class="el-icon-date"></i>{{ allVideoCount.barrageCount }}</span>
              <span style="margin-left: 20px"><i class="el-icon-timer"></i>{{ allVideoCount.uploadDate }}</span>
            </div>
          </el-col>
          <el-col :span="8">
            <div style="margin-left: 145px;width: 400px;">
              <el-card styly="height: 20px">
                <div style="display: flex">
                  <div style="margin-left: 20px">
                    <el-popover placement="bottom" trigger="hover" width="300">
                      <el-col :span="12">
                        <span><i class="el-icon-user"></i><span>粉丝数:</span></span>
                        <span>{{ allVideoCount.fans }}</span>
                      </el-col>
                      <el-col :span="12">
                        <span><i class="el-icon-user"></i><span>关注数:</span></span>
                        <span>{{ allVideoCount.follow }}</span>
                      </el-col>
                      <el-button slot="reference">
                        <el-avatar :src='allVideoCount.imgUrl'
                                   style="width: 40px;height: 40px"></el-avatar>
                      </el-button>
                    </el-popover>
                  </div>
                  <div style="margin-left: 40px">
                    <h4>{{ allVideoCount.nickname }}</h4>
                    <div style="margin-top: 5px ">
                      <el-button icon="el-icon-success" size="mini" type="primary"
                                 @click.native="jump(allVideoCount.userID)">
                        查看更多
                      </el-button>
                    </div>
                  </div>
        ...
        <el-row :gutter="0">
          <el-col :span="18">
            <div style="width: 1040px">
              ...
              <!-- <div id="mui-player">
                               <div id="comment-stage" class='container' style="z-index: 1 !important;height:375px;width: 100%;"></div>
              </div> -->

              <!-- 视频简介标签栏 -->
              <el-card style="width: 1040px;margin-top: 18px">
                <div>
                  <p>{{ allVideoCount.description }}</p>
                </div>
                <el-divider></el-divider>
                <div>
                  <span v-for="t in tag">
                    <el-tag style="margin-left: 5px" type="success">{{ t.tagName }}</el-tag>
              ...
              <div style="margin-top: 20px">
                <el-row :gutter="50">
                  <el-col :span="2">
                    <div>
                      <div>
                        <el-button v-if="favoriteClick" style="border: 0" @click="addfavorite()">
                          <i class="iconfont icon-shoucang" style="font-size: 50px"></i>
                          {{ allVideoCount.favoriteCount }}
                        </el-button>
                        <el-button v-else @click="deletefavorite()" style="border: 0">
                          <i class="iconfont icon-shoucang-copy" style="font-size: 50px;color: #20aee5"></i>
                          {{ allVideoCount.favoriteCount + 1 }}
                        </el-button>
                      </div>
                    </div>
                  </el-col>
                  ...
                  <el-col :span="2">
                    <div>
                      <!-- 使用v-if指令来动态显示或隐藏不同的按钮 -->

                      <el-button @click="handleLike()" v-if="isClicked" style="border: 0">
                        <i class="iconfont icon-dianzan" style="font-size: 50px"></i>
                        {{ allVideoCount.likeCount }}
                      </el-button>

                      <el-button @click="removeLike()" style="border: 0" v-else>
                        <i class="iconfont icon-icon" style="font-size: 50px;color: #20aee5"></i>
                        {{ allVideoCount.likeCount + 1 }}
                      </el-button>
                    </div>
                  </el-col>
                  ...
                  <el-col :span="2">
                    <div>
                      <el-button @click="handleDislike()" v-if="DisClicked" style="border: 0">
                        <i class="iconfont icon-dianzan-copy" style="font-size: 50px"></i>
                        {{ allVideoCount.dislikeCount }}
                      </el-button>
                      <el-button @click="removeDislike()" v-else style="border: 0">
                        <i class="iconfont icon-icon-copy" style="font-size: 50px;color: #20aee5"></i>
                        {{ allVideoCount.dislikeCount + 1 }}
                      </el-button>
              ...
              <div>
                <el-divider></el-divider>
                <!-- flex布局一个<div>放头像,一个<div>放名字-->
                <div v-for="(item, index) in videoComments" :key="index" style="display: flex;">
                  <!--                    头像div-->
                  <div style="width: 5%;">
                  ...
                  <div style="width: 90%;">
                    <!--                      名字-->
                    <div style="color: #FB7299;font-size: 14px;">{{ item.nickname }}</div>
                    <!--                      内容-->
                    <div style="color: #18191c;font-size: 16px;margin-top: 10px;">{{
                        item.commentText
                      }}
                    </div>

                    <!--                      第三个div --- 点赞数,点踩,回复按钮 -->
                    <div style="display: flex;font-size: 13px;color: #9499A0;margin-top: 10px;">
                      <div>{{ item.gmtCreate }}</div>
                      <!-- 点赞数 -->
                      <div style="margin-left: 20px;cursor: pointer;"><i class="el-icon-top"></i>
                        <span v-if="item.likeCount">{{ item.likeCount }}</span>
                      </div>
                      <!-- 点踩数 -->
                      <div style="margin-left: 20px;cursor: pointer;"><i class="el-icon-bottom"></i><span
                          v-if="item.dislikeCount">{{ item.dislikeCount }}</span></div>
                      <!--                        回复按钮-->
                      <div style="margin-left: 20px;cursor: pointer;">
                        <el-popover placement="right" width="300" trigger="click">
                          <el-form :inline="true" :model="formInline" class="demo-form-inline" style="width: 350px">
                            <el-form-item label="">
                              <el-input type="textarea" autosize placeholder="请输入内容" v-model="formInline.textarea1"
                                        maxlength="300">
                              </el-input>
                            </el-form-item>
                            <el-form-item>
                              <el-button type="primary" @click="onSubmit1(item.commentID, 0)">提交</el-button>
                            </el-form-item>
                          </el-form>
                    ...
                    <!--                      在评论下面写回复-->
                    <!-- 回复 -->
                    <div v-for="(it, index) in item.commentList" v-if="item.isShow ? index < 11 : index < 3"
                         :key="index"
                         style="display: flex;margin-top: 10px;">
                      <div style="width: 5%;">
                        <el-avatar
                            :src='it.imgUrl'></el-avatar>
                      </div>
                      <div style="width: 90%;">
                        <!--                          用户名1 -->
                        <span style="color: #FB7299;font-size: 16px;">{{ it.nickname }}</span>
                        <!--                          回复按钮-->
                        <span v-if="it.pnickname">
                          回复 @
                          <!--                            用户名2-->
                          <span style="color: #008AC5;">{{ it.pnickname }}</span>:
                        </span>

                        <!--                          回复内容-->
                        <span style="margin-left: 10px;line-height: 1.8;">{{ it.replyText }}</span>
                        <div style="display: flex;font-size: 13px;color: #9499A0;margin-top: 10px;">
                          <div>{{ it.gmtCreate }}</div>
                          <!-- 点赞数 -->
                          <div style="margin-left: 20px;cursor: pointer;"><i class="el-icon-top"></i> <span
                              v-if="item.likeCount > 0">{{ it.likeCount }}</span></div>
                          <!-- 点踩数 -->
                          <div style="margin-left: 20px;cursor: pointer;"><i class="el-icon-bottom"></i><span
                              v-if="item.dislikeCount > 0">{{ it.dislikeCount }}</span>
                          </div>
                          <div style="margin-left: 20px;cursor: pointer;">
                            <el-popover placement="right" width="300" trigger="click">
                              <el-form :inline="true" :model="formInline" class="demo-form-inline">
                                ...
                                <el-form-item>
                                  <el-button type="primary" @click="onSubmit1(item.commentID, it.replyID)">提交
                                  </el-button>
                                </el-form-item>
                              </el-form>
                              <el-button slot="reference">回复</el-button>
                    ...
                    <!--                      点击查看-->
                    <!--                       v-if : "当我们大于三条回复的时候才查看..."-->
                    <div v-if="item.commentListLength > 3 && !item.isShow"
                         style="font-size: 13px;color: #9499A0;margin-top: 20px;">
                      共{{ item.commentListLength }}条回复, <span style="cursor: pointer;"
                                                                  @click="showContentDetail(item.commentID)">点击查看</span>
                    </div>

                    <el-divider></el-divider>
                    <!-- <div style="height: 1px;background-color: #DCDFE6;margin: 10px 0;"></div> -->
                  </div>
          ...
          <el-col :span="6">
            ...
            <!-- 相关推荐 -->
            <div class="infinite-list-wrapper" style="overflow:auto;width: 600px">
              <ul v-infinite-scroll="load" class="list" infinite-scroll-disabled="disabled">
                <li v-for="i in videoCF" class="list-item">
                  <el-card class="zoom" style="width: 400px;margin-top: 10px;margin-left: 20px"
                           @click.native="jump1(i.videoID)">
                    <img :src='i.videoImgUrl' style="width: 320px;height: 180px;margin-left: 20px">
                    <p>{{ i.title }}</p>
                  </el-card>
                </li>
              </ul>
...
import otherSpace from "@/views/personal/otherSpace.vue";
...
export default {
  ...
  data() {
    return {
      ...
      //   评论列表
      videoComments: [],
      videoComment: {videoID: "", commentText: ""},
      // 播放器相关参数
      videoPlayer: {},
      isMeDanmu: '',
      ...
      videoSrc: [
        ...
        // 'https://youbili-test.oss-cn-beijing.aliyuncs.com/3-89/3-89-480.mp4',//480P
      ],
      videoNumber: 0
    };
  },
  ...
  created() {
    this.videoData = this.$route.query.videoID;
    console.log(this.videoData)
    //获取弹幕列表
    this.loadBarrage(this.videoData)
    // 获取评论数
    this.reloadComments(this.videoData)
    //获取所有计数
    // this.reloadComments()
    //获取视频页面所有信息
    this.videoAll(this.videoData)
    //获取视频页面推荐视频
    this.videoCFlist();
    //获取视频标签
    this.getvideoTag(this.videoData);
  },
  ...
  method: {
    reply(pid, target) {
      this.comment = {pid: pid, UserId: '1', username: '李四', foreignId: 1, target: target}
  ...
  methods: {
    //获取标签信息
    getvideoTag(videoID) {
      getvideoTag({videoID: videoID}).then(res => {
        if (res.success) {
          this.tag = res.data
    ...
    // 点赞
    handleLike() {
      forLike({videoID: this.videoData}).then(res => {
        // 如果请求成功，打印响应数据
...
//取消点赞
    removeLike() {
      deleteLike({videoID: this.videoData}).then(res => {
        // 如果请求成功，打印响应数据
        console.log(res.data);
        // 显示一个成功的消息
        this.isClicked = true;
    ...
    //点踩
    handleDislike() {
      ForDislike({videoID: this.videoData}).then(res => {
        // 如果请求成功，打印响应数据
        console.log(res.data);
        // 显示一个成功的消息
        if (res.status == 20000) {
    ...
    //取消点踩
    removeDislike() {
      deleteDisLike({videoID: this.videoData}).then(res => {
        // 如果请求成功，打印响应数据
        console.log(res.data);
        // 显示一个成功的消息
        this.DisClicked = true;
      })
          .catch(err => {
            // 如果请求失败，打印错误信息
            console.error(err);
    ...
    loadBarrage(videoID) {
      LoadBarrage({videoID: videoID}).then((response) => {
        if (response.success) {
    ...
    sendBarrage(opts, v) {
      let BarrageDTO = {...opts, ...v};
      SendBarrage(BarrageDTO).then((response) => {
        if (response.success) {
          this.$notify.success({
            position: 'bottom-right',
            title: '发送成功',
    ...
    reloadComments(videoID) {
      getComment(videoID, 1).then(response => {
        if (response.success) {
          this.videoComments = response.data.list
        } else {
          this.$message.error(response.message)
        }
      })
    ...
    onSubmit1(a, b) {
      this.send.commentID = a
      this.send.parentReplyID = b
      this.send.replyText = this.formInline.textarea1
      console.log(this.send)
      insertComment(this.send).then(res => {
    ...
    videoAll(videoID) {
      allCount({videoID: videoID}).then(res => {
        if (res.success) {
          ...
          this.videoSrc = [];
          this.videoSrc.push(this.resolution_1080p)
          this.videoSrc.push(this.resolution_720p)
          this.videoSrc.push(this.resolution_480p)
    ...
    //添加收藏
    addfavorite() {
      addfavorite({videoID: this.videoData}).then(res => {

        if (res.success) {
          this.favoriteClick = false;
          this.$notify.success({
            position: 'bottom-right',
            title: '添加收藏成功'
          })
    ...
    //删除收藏
    deletefavorite() {
      deleteVideofavorite({videoID: this.videoData}).then(res => {
        if (res.success) {
    ...
    showContentDetail(id) {
      this.videoComments.find(item => item.commentID == id).isShow = true
    },
    test() {
    ...
    nPlayer() {
      ...
      // 注册图标
      const registerIcon = Player.Player.Icon.register
      registerIcon('volume', this.createIcon(volume))
      registerIcon('cog', this.createIcon(cog))
      registerIcon('webEnterFullscreen', this.createIcon(webFull))
      registerIcon('enterFullscreen', this.createIcon(full))

      // 弹幕内容
      const danmakuOptions = {
        items: this.item
        // { time: 5, text: '弹幕1～', color: '#FE0302',type: 'top' },
      ...
      const copyVideoUrl = {
        ...
        click(player) {
          ...
          navigator.clipboard.writeText(player.video.currentSrc)
        }
      }
      ...
      this.videoPlayer = new Player({
        src: this.videoSrc[this.videoNumber], // 视频地址
        contextMenus: ['loop', 'pip', copyVideoUrl], // 右键菜单设置项
        plugins: [new Danmaku(danmakuOptions)], // 弹幕配置项
        settings: [Mirroring, Picture, 'speed'],
        controls: [
          [
            "play",
            "volume",
        ...
        volumeVertical: true,
        progressDot: this.createIcon(dot, true)(),
        posterPlayEl: this.createIcon(playBig)(),
      })

      this.videoPlayer.mount('#nPlayer')

      this.videoPlayer.on('DanmakuSend', opts => {
        // sendMsg(opts)

        this.sendBarrage(opts, {videoID: this.videoData})
      })
    },
    createIcon(html, noCls) {
    ...
    // 修改画质
    changeVideo() {
      this.videoPlayer.video.src = this.videoSrc[this.videoNumber]
    }
...
#nPlayer >>> .nplayer_video-mirroring {
  transform: scaleX(-1);
}
</style>